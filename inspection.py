...
def parse_disks(output: str) -> list:
    """Parse every filesystem reported by POSIX ``df -P`` output."""
    disks = []
    lines = output.splitlines()
    for line in lines[1:]:
        parts = [p for p in line.split() if p]
        if len(parts) < 6 or not parts[4].endswith("%"):
            continue
        filesystem, mount = parts[0], parts[5].replace("\\040", " ")
        try:
            usage = float(parts[4][:-1])
        except ValueError:
            continue
        disks.append({"filesystem": filesystem, "mount": mount, "usage": usage})
    return disks


def parse_disk(output: str) -> float:
    """Return the highest usage for backward-compatible report/status fields."""
    return max((item["usage"] for item in parse_disks(output)), default=0.0)
