...
def generate_pdf_report(project_name: str, inspector: str, date_str: str, rows: List[Dict[str, Any]], proxy_results: List[Dict[str, Any]] = None, check_cpu: bool = True, check_mem: bool = True, check_disk: bool = True, task_name: str = None) -> str:
    ...
    for i in range(1, len(table_data)):
        row = table_data[i]
        if row[status_col] == "正常":
            table_style.append(('TEXTCOLOR', (status_col, i), (status_col, i), colors.green))
        elif "告警" in row[status_col]:
            table_style.append(('TEXTCOLOR', (status_col, i), (status_col, i), colors.orange))
        else:
            table_style.append(('TEXTCOLOR', (status_col, i), (status_col, i), colors.red))
